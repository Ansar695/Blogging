...
export async function generateMetadata({ params }) {
    ...
    return {
      ...
      openGraph: {
        title: blog?.title,
        description: blog?.description,
        url: siteMetadata.siteUrl + blog.description,
        ...
        <article>
            <div className="mb-8 text-center relative w-full h-[70vh] bg-dark">
                <div className="w-full z-10 flex flex-col items-center justify-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                    ...
                    />
                    <h1 className="inline-block mt-6 font-semibold capitalize text-light text text-2xl md:text-3xl lg:text-5xl leading-normal relative w-5/6">
                        {blog?.title}
                    </h1>
                </div>
                                ...
                                return (
                                    <li key={`#${heading.slug}`} className="py-1">
                                        <a href={`#${heading?.slug}`} 
                                            ...
                                            "
                                        >
                                            {heading.level===3 ? <span className="flex w-1 h-1 rounded-full bg-dark mr-2">&nbsp;</span> : null}
